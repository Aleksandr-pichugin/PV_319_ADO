...
namespace AcademyDataset
{
	...
	public void AddTable(string table, string columns)
		...
		{
			DataTable parent_table = Set.Tables[parent.Split(',')[0]];
			DataTable child_table = Set.Tables[child.Split(',')[0]];
			Set.Relations.Add
					(
					name,
		...
		public void LoadGroupsRelatedData()
		{
			...
			Set.Relations.Add
				...
				Set.Tables["Directions"].Columns["direction_id"], //Parent field (Primary key) в другой таблице
				Set.Tables["Groups"].Columns["direction"]         //Child field  (Foriegn key) в нашей таблице
				);
			...
			foreach (DataRow row in Set.Tables[dsTable_Groups].Rows)
			{
				Console.WriteLine
						(
						$"{row[dst_Groups_col_group_id]}" +
						$"\t{row[dst_Groups_col_group_name]}" +
						$"\t{row.GetParentRow(dsRelation_GroupsDirections)[dst_col_direction_name]}"
						);
			}
			Console.WriteLine("\n=====================================\n");
		}
		public void Print(string table)
		{
			...
			if (hasParents(table))
			{
				...
				//	Console.WriteLine(parent_column.);
				parent_index =
			...
			foreach (DataRow row in Set.Tables[table].Rows)
			{
				...
				////Console.WriteLine(parent_row[j] + "\t" );
		...
		bool hasParents(string table)
		{
			...
			//{
			//	if (GroupsRelatedData.Relations[i].ChildTable.TableName == table)
			//	return true;
			//}
			//return yes;
		}
